...
static bool readIntegerTree(RBTree<int, int>& tree, FILE* f);
...
static bool writeIntegerTree(
    const Node<int, int>* root, FILE* f, int level /* = 0 */
) {
    int i;
    for (i = 0; i < level; ++i) {
        fprintf(f, "    ");
    ...
    if (root == 0) {
        ...
        Integer<int>* v = (Integer<int>*) root->value;
...
static bool readIntegerTree(RBTree<int, int>& tree, FILE* f) {
    ...
    if (i < len && (isdigit(line[i]) || line[i] == '-')) {
        ...
        RBTree<int, int> leftSubtree;
        if (!readIntegerTree(leftSubtree, f))   // recursive call
            return false;

        RBTree<int, int> rightSubtree;
        if (!readIntegerTree(rightSubtree, f))  // recursive call
            return false;

        Node<int, int>* rootNode = new Node<int, int>();
        rootNode->red = red;
        rootNode->value = new Integer<int>(n);
